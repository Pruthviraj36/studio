...
const GithubStats = dynamic(
  () => import("@/components/github-stats").then((mod) => mod.GithubStats),
  { ssr: false },
);
...
export default function ProfilePage() {
  ...
  const [editExperience, setEditExperience] =
    useState<User["experience"]>("Beginner");
    ...
    try {
      ...
      if (result.success) {
        toast({
          ...
          description:
            "Your GitHub snapshot has been updated with real-time data.",
        });
        fetchProfile();
      } else {
      ...
      toast({
        ...
        description:
          error.message || "Could not fetch GitHub data. check your URL.",
        variant: "destructive",
      });
    } finally {
      setRefreshing(false);
  ...
  return (
    <ResponsiveContainer maxWidth="xl" padding="px-4 py-4">
      <div
        className={
          isMobile ? "space-y-6" : "grid grid-cols-1 gap-8 lg:grid-cols-3"
        }
      >
        <div className={isMobile ? "space-y-6" : "lg:col-span-1 space-y-8"}>
          ...
          <Card className="bg-white/20 dark:bg-black/20 backdrop-blur-md">
            <CardContent
              className={isMobile ? "p-4 text-center" : "p-6 text-center"}
            >
              <Avatar
                className={
                  isMobile
                    ? "h-24 w-24 mx-auto mb-3 border-4 border-primary/50"
                    : "h-32 w-32 mx-auto mb-4 border-4 border-primary/50"
                }
              >
                <AvatarImage
              ...
              <p
                className={
                  isMobile ? "mt-2 text-xs line-clamp-3" : "mt-4 text-sm"
                }
              >
                {user.bio}
              </p>
              ...
              >
                <DialogTrigger asChild>
                  <Button
                    ...
                    className={
                      isMobile
                        ? "mt-3 w-full bg-white/50 dark:bg-black/50 h-11"
                        : "mt-4 w-full bg-white/50 dark:bg-black/50"
                    }
                  >
